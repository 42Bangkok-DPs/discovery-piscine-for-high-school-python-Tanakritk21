#!/usr/bin/env python3
def checkmate(board):
    # Convert the board string into a 2D list for easier manipulation
    board = [list(row) for row in board.splitlines()]
    n = len(board)  # Get the size of the board
    king_pos = None

    # Find the king's position (assuming there is only one king on the board)
    for i in range(n):
        for j in range(n):
            if board[i][j] == 'K':
                king_pos = (i, j)
...
def pawn_moves(board, x, y, king_x, king_y):
    ...
    if board[x][y] == 'P':
        # Assuming pawns are black for downward movement
